(function () {
  ...
  function buildPdfSummaryData({ month, year, snapshots }) {
    ...
    return { rows, monthLabel: MONTHS[month - 1] || String(month), year };
  }
  ...
  function exportSummaryPdf(summary) {
    ...
    const head = [['Sector', 'Han', 'DS', 'DF', 'DJM', 'DJF', 'Total', 'Comentario']];
    const body = summary.rows.length
      ? summary.rows.map((row) => [row.sector, row.han, String(row.ds), String(row.df), String(row.djm), String(row.djf), String(row.total), row.comentario || ''])
      : [['-', '-', '0', '0', '0', '0', '0', 'Datos no cargados']];
