(function () {
  ...
  function divisionBucket(division) {
    const raw = String(division || '')
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toUpperCase()
      .trim();
    if (raw === 'DS') return 'DS';
    if (raw === 'DD' || raw === 'DF') return 'DF';
    if (raw === 'DJM') return 'DJM';
    if (raw === 'DJF') return 'DJF';
    return 'OTRA';
  }
  ...
  function buildPdfSummaryData({ month, year, snapshots }) {
    ...
    (hanes || []).forEach((han) => {
      ...
      if (!snapshot) {
        rows.push({
          ...
          djf: 0,
          total: 0,
          comentario: 'Datos no cargados'
        });
        return;
      }

      const divisionCounts = { DS: 0, DF: 0, DJM: 0, DJF: 0, OTRA: 0 };
      let totalChecked = 0;
      (snapshot.entries || []).forEach((entry) => {
        if (!hasAnyCheck(entry)) return;
        totalChecked += 1;
        const bucket = divisionBucket(resolveDivisionForEntry(entry));
        divisionCounts[bucket] = (divisionCounts[bucket] || 0) + 1;
      });
      ...
      rows.push({
        sector: han.sector || 'Sin sector',
        han: han.name || hanId,
        ds: divisionCounts.DS || 0,
        df: divisionCounts.DF || 0,
        djm: divisionCounts.DJM || 0,
        djf: divisionCounts.DJF || 0,
        total: totalChecked,
        comentario: ''
      });
    });
    ...
    rows.sort((a, b) => {
      const sectorCmp = a.sector.localeCompare(b.sector, 'es');
      if (sectorCmp !== 0) return sectorCmp;
      return a.han.localeCompare(b.han, 'es');
    });

    const rowsWithSectorTotals = [];
    let currentSector = '';
    let sectorAcc = { ds: 0, df: 0, djm: 0, djf: 0, total: 0 };

    function pushSectorTotalRow(sectorName) {
      rowsWithSectorTotals.push({
        sector: sectorName,
        han: `TOTAL ${sectorName}`,
        ds: sectorAcc.ds,
        df: sectorAcc.df,
        djm: sectorAcc.djm,
        djf: sectorAcc.djf,
    ...
    rows.forEach((row, index) => {
      if (index === 0) currentSector = row.sector;
      if (row.sector !== currentSector) {
        pushSectorTotalRow(currentSector);
        sectorAcc = { ds: 0, df: 0, djm: 0, djf: 0, total: 0 };
        currentSector = row.sector;
      }

      rowsWithSectorTotals.push({ ...row, isSectorTotal: false });
      sectorAcc.ds += Number(row.ds || 0);
      sectorAcc.df += Number(row.df || 0);
      sectorAcc.djm += Number(row.djm || 0);
      sectorAcc.djf += Number(row.djf || 0);
      sectorAcc.total += Number(row.total || 0);
    });

    if (rows.length) pushSectorTotalRow(currentSector);

    return { rows: rowsWithSectorTotals, monthLabel: MONTHS[month - 1] || String(month), year };
  }
  ...
  function exportSummaryPdf(summary) {
    ...
    const body = summary.rows.length
      ? summary.rows
      : [{ sector: '-', han: '-', ds: 0, df: 0, djm: 0, djf: 0, total: 0, comentario: 'Datos no cargados', isSectorTotal: false }];
    ...
    if (typeof doc.autoTable === 'function') {
      doc.autoTable({
        ...
        columns: [
          ...
          { header: 'Han', dataKey: 'han' },
          { header: 'DS', dataKey: 'ds' },
          { header: 'DF', dataKey: 'df' },
          { header: 'DJM', dataKey: 'djm' },
          { header: 'DJF', dataKey: 'djf' },
        ...
        didParseCell: (hookData) => {
          ...
          if (hookData.section === 'body' && raw?.isSectorTotal) {
            hookData.cell.styles.fillColor = [31, 122, 140];
      ...
      let y = 90;
      body.forEach((row) => {
        doc.text([row.sector, row.han, row.ds, row.df, row.djm, row.djf, row.total, row.comentario || ''].join(' | '), 40, y);
        y += 14;
      });
    }
