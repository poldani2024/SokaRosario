(function () {
  ...
  function buildAuditEvent(action) {
    ...
    return {
      ...
      byEmail: user?.email || '',
      atIso: nowIso,
      at: window.firebase?.firestore?.FieldValue?.serverTimestamp ? window.firebase.firestore.FieldValue.serverTimestamp() : nowIso,
    };
  }
    ...
    if (window.db) {
      ...
      if (snap.exists) {
        ...
        currentSnapshot = {
          ...
          createdAt: window.firebase?.firestore?.FieldValue?.serverTimestamp ? window.firebase.firestore.FieldValue.serverTimestamp() : new Date().toISOString(),
