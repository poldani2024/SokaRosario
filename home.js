...
(function () {
  ...
  function isMiembroStatus(status) {
  ...
  function computeDivisionSummary({ personas, stats }) {
    const miembros = (personas || []).filter((p) => isMiembroStatus(p?.status));
  ...
  async function refreshControlBoard() {
    ...
    renderDashboard(summary, { hanes: hanes.length, miembros: summary.miembrosCount });
  }
