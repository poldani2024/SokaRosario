...
(function () {
  ...
  async function loadAndRenderGrupos() {
    try {
      ...
      grupos = Array.isArray(fsList) ? fsList : loadGruposLS();
      if (Array.isArray(fsList)) saveGruposLS(grupos);
    } catch (err) {
      console.warn('[grupos] Firestore no disponible, usando localStorage.', err);
      grupos = loadGruposLS();
    }
    renderGruposTable();
  }
  ...
  async function onSaveGrupo() {
    ...
    try {
      let savedId = null;
      if (window.db) savedId = await saveGrupoFS({ id: editGrupoId, ...vals });
      if (!savedId) {
        ...
        if (editGrupoId) {
          ...
          editGrupoId = list[list.length - 1].id;
        }
        saveGruposLS(list);
      }
    ...
    if (tbody) {
      tbody.addEventListener('click', (e) => {
        ...
        if (btn) {
          ...
          if (action === 'delete') onDeleteGrupo(id);
          return;
        }
        const tr = e.target.closest('tr[data-id]');
        if (tr?.dataset?.id) onEditGrupo(tr.dataset.id);
      });
    }
  });
})();
