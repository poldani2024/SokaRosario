(function () {
  ...
  function renderMembers() {
    ...
    const membersGridWrap = document.createElement('div');
    membersGridWrap.className = 'member-table-wrap';
    ...
    membersGridWrap.appendChild(table);
    host.appendChild(membersGridWrap);
    renderMemberMode();
  }
  ...
  function renderPolicyFieldChecklist() {
    const host = $('policyFieldChecklist');
    if (!host) return;
    host.innerHTML = '';
    PERSONA_FIELDS.forEach((field) => {
  ...
  function selectedBulkFieldKeys() {
    return Array.from(document.querySelectorAll('.bulk-field-cb:checked')).map((el) => el.value);
  }

  function applyBulkPolicies() {
    const fields = selectedBulkFieldKeys();
    if (!fields.length) {
      alert('Seleccioná al menos un campo.');
      return;
    }
    const node = currentNode();
    const responsable = $('bulkResPolicy')?.value || 'ver_editar';
    const viceResponsable = $('bulkVicePolicy')?.value || 'solo_ver';
    const inheritance = $('bulkInheritance')?.value || 'local';

    fields.forEach((fieldKey) => {
      const existing = node.policies.find((p) => p.field === fieldKey);
      if (existing) {
        existing.responsable = responsable;
        existing.viceResponsable = viceResponsable;
        existing.inheritance = inheritance;
      } else {
        node.policies.push({ field: fieldKey, responsable, viceResponsable, inheritance });
      }
    });
    persistLocal();
    renderPolicies();
    dump('Políticas aplicadas a campos seleccionados', { fields, responsable, viceResponsable, inheritance });
  }
  ...
  function bindEvents() {
    ...
    $('applyBulkPolicyBtn')?.addEventListener('click', applyBulkPolicies);
    $('bulkSelectAllFields')?.addEventListener('change', (e) => {
      const checked = !!e.target.checked;
      document.querySelectorAll('.bulk-field-cb').forEach((cb) => {
        cb.checked = checked;
      });
    });
